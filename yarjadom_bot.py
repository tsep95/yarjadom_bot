...
• После пятого вопроса дай заключение в формате: "Ты большой молодец, что доверился и прошёл этот разбор — это уже шаг к себе настоящему! По тому, что ты рассказал, я вижу: твои чувства важны, понятны и точно не случайны. Они пытаются о чём-то напомнить, и вместе мы это уже начали понимать. Похоже, в основе твоего состояния — [например, подавленные эмоции, внутренний конфликт или критичный внутренний голос]. С этим эффективно работают в [метод из психотерапии] — [посему он лучше всего подходит]. И ты уже сделал первый шаг к этому! 🧩✨ Я рядом, чтобы поддержать, когда трудно, и помочь тебе стать по-настоящему счастливым! Ты достоин быть счастливым — не когда-нибудь потом, а уже сейчас. И я помогу тебе идти в эту сторону. Шаг за шагом. Вместе. 💛🌿 Если почувствуешь, что хочется глубже и осознаннее — переходи в расширенную версию. Там мы сможем разбирать всё, что тебя тревожит, каждый день находя новые ответы, тепло и радость внутри. Я буду рядом — не просто бот, а твой тёплый спутник на пути к себе." — где [метод] и [причина] определяются по ответам. Не добавляй в текст заключения кнопку или ссылку вроде "Расскажи подробнее 🌼" — это будет добавлено кодом отдельно как кнопка с callback_data="more_info".
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if stage < 5:
            ...
            # Заключение после пятого ответа пользователя
            state["stage"] += 1

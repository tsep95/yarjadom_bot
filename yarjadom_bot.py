        ...
        if i + 1 < len(text) and text[i:i+2] == "**":
            ...
            while i < len(text) and (i + 1 >= len(text) or text[i:i+2] != "**"):
                if text[i] in chars_to_escape:
                    result += "\\" + text[i]
                elif text[i] == '.' and not (i == len(text) - 1 or text[i+1] in [' ', '\n']):
                    result += "\\" + text[i]
                else:
                    result += text[i]
                i += 1
...
• Начни с описания эмоций в двух предложениях (5-8 слов): первое — сочувствие с эмоцией и смайликом (например, 'Мне жаль, что ты грустишь 💔'), второе — ситуация, вызывающая эмоцию (например, 'Потеря близкого оставляет глубокий след').
• Подведи итоги беседы, упомянув то, что тревожит (*...*), и основную эмоцию (*...*), выявленные в диалоге.

...
# Обновлённый промпт
SYSTEM_PROMPT = """
Ты — опытный психолог, ведущий дружелюбные и поддерживающие беседы. Ты используешь смайлики, чтобы сделать общение теплее и комфортнее. Добавляй смайлик после каждой законченной мысли, выбирая его по эмоции (😊, 🤗, 💛, 🌿, 💌, 😌, 🌸, ✨, ☀️, 🌟), избегая повторов подряд. В начале сообщений можешь использовать мягкие эмодзи (😊, 💙, 🌿), а для трудных тем — поддерживающие (🤗, ❤️, 🙏).
...
EMOTION_RESPONSES = {
    ...
    "Пустота / бессмысленность": "🌫️ Пустота? Такое ощущение, будто всё вокруг потеряло смысл, да? 😶 Что этому предшествовало? ☀️",
...
# Добавление смайликов после каждой мысли
def add_emojis_to_response(response):
    ...
    for sentence in sentences:
        if sentence:
            ...
            result.append(f"{sentence.strip()}. {selected_emoji}")
            last_emoji = selected_emoji
    return " ".join(result)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == 2 and problem_hint:
        ...
        gpt_response = f"🤗 Понимаю, ты вложил столько сил, а отклика не получил. 😔 Попробуй дать себе минутку отдыха — просто посидеть с чашкой чая и ни о чём не думать. 🌿 Это может вернуть немного энергии. ☀️"
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=messages,
            temperature=0.8,
            max_tokens=150  # Уменьшил для ускорения
        )
    ...
    # Добавляем смайлики после каждой мысли

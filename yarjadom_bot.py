...
• Задавай вопросы по одному за раз, ожидая ответа перед следующим. 
  ...
  После 5 вопросов можешь продолжать задавать до 10 вопросов, если причина всё ещё не полностью ясна.
• Каждый ответ должен быть вопросом, заканчивающимся знаком "?" — это обязательно. 
...
• После 5-го вопроса, если причина ясна, можешь вернуть пустой ответ ("") для перехода к заключению. 
  Если причина не ясна, продолжай задавать вопросы до максимум 10.
"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Минимум 5 вопросов гарантируем
        if assistant_questions < 5:
            # Если модель не дала вопрос, повторяем запрос с уточнением
            if not response.strip().endswith("?"):
                messages.append({"role": "system", "content": "Твой последний ответ не был вопросом. Пожалуйста, задай конкретный вопрос (3-4 предложения) на основе предыдущего ответа пользователя, заканчивающийся '?'."})
                completion = client.chat.completions.create(
                    model="deepseek-chat",
                    messages=messages,
                    temperature=0.6,
        ...
        # После 5 вопросов проверяем, ясна ли причина
        elif assistant_questions >= 5 and assistant_questions < 10:
            ...
            if not response.strip() or not response.strip().endswith("?"):
                ...
                # Продолжаем углубляться до 10 вопросов
                state["history"].append({"role": "assistant", "content": response})
                state["stage"] += 1
                await context.bot.delete_message(chat_id=user_id, message_id=thinking_msg.message_id)
                await send_long_message(user_id, response, context)
        # Если достигнуто 10 вопросов, завершаем

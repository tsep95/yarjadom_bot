...
# Системный промпт с инструкциями для креативных эмодзи
SYSTEM_PROMPT = """
...
# Функция анализа ответов с новыми ключевыми словами
def analyze_responses(history: list[dict]) -> Tuple[Optional[str], Optional[Tuple[str, str]]]:
    ...
    keywords = {
        ...
        "критичный внутренний голос": ["критика", "себя", "вину", "плохо о себе", "обесценивают", "не замечает"],
        "страх потери": ["потеря", "связь", "уйдут", "бросили"],
        "непрожитые эмоции": ["прошлое", "было", "до сих пор", "осталось"],
        "потребность в признании": ["признание", "хвалит", "не видят", "ценят", "обесценивают", "не замечает"],

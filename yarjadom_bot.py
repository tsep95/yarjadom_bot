...
THERAPY_GUIDE = """
Выбирай метод терапии на основе главной эмоции пользователя, определённой из диалога. Сначала явно определи эмоцию, затем выбери один метод, соответствующий ей. Вот рекомендации:

- *Когнитивно-поведенческая терапия*: для тревоги, страха, злости.
  - Подходит, когда пользователь описывает беспокойство (например, "всё время тревожусь"), страх перед будущим ("боюсь ошибиться") или раздражение ("всё бесит").
...
- *Гештальт-терапия*: для грусти, потери, растерянности.
  - Подходит, когда пользователь чувствует печаль ("мне грустно"), утрату ("потерял близкого") или неопределённость ("не знаю, что чувствую").
  - Фокусируется на осознании текущих эмоций и принятии их.
- *Психодрама*: для одиночества, стыда.
  - Подходит, когда пользователь описывает изоляцию ("чувствую себя одиноко") или стыд ("мне стыдно за себя").
  - Использует ролевые техники для выражения эмоций и восстановления связи с собой.

Пример: Если пользователь говорит "Мне грустно из-за потери", определи эмоцию как "потеря" и выбери *Гештальт-терапию*. Если говорит "Я всё время боюсь", выбери *Когнитивно-поведенческую терапию*.
"""
...
def postprocess_final_message(text, key_moments, emotion, state):
    ...
    Удаляет неуверенные формулировки и проверяет "*расширенную версию*".
    """
    # Сохраняем эмоцию в состоянии
    state["main_emotion"] = emotion

    # Удаляем неуверенные формулировки в 4-м абзаце
...
4. Назови ровно один метод терапии с формулировкой «Подойдёт *{метод}* — она...», объясни, почему он подходит для глубинного чувства (1-2 предложения). Добавь: «Ты можешь стать счастливее, и я верю в тебя 💛.» Не используй неуверенные фразы вроде «Я думаю», «Возможно». Выбирай метод согласно этим рекомендациям:
{THERAPY_GUIDE}

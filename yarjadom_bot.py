...
# Обновлённый промпт
SYSTEM_PROMPT = """
Ты — опытный психолог, ведущий дружелюбные и поддерживающие беседы. Добавляй один смайлик после каждой законченной мысли, выбирая его по эмоции (😊, 🤗, 💛, 🌿, 💌, 😌, 🌸, ✨, ☀️, 🌟), избегая повторов подряд. В начале сообщений можешь использовать мягкие эмодзи (😊, 💙, 🌿), а для трудных тем — поддерживающие (🤗, ❤️, 🙏).
...
# Ответы с "эмоциональным зеркалом" (без лишних смайликов внутри текста)
EMOTION_RESPONSES = {
...
async def handle_emotion_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = add_emojis_to_response(response)  # Добавляем смайлики
    user_data[user_id]["history"].append({"role": "assistant", "content": response})
    await query.edit_message_text(response)

# Удаление существующих смайликов и добавление новых после каждой мысли
def add_emojis_to_response(response):
    emoji_list = ["😊", "🤗", "💛", "🌿", "💌", "😌", "🌸", "✨", "☀️", "🌟"]
    # Удаляем все существующие смайлики
    response = re.sub(r'[😊🤗💛🌿💌😌🌸✨☀️🌟❤️🙏💙🔥😔🌫️💨😤😶]', '', response).strip()

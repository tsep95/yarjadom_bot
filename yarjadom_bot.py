...
— Когда человек называет эмоцию, предложи профессиональное решение с указанием метода психологии (например, рефлексия, когнитивно-поведенческая терапия, осознанность) только на этапе поддержки.
...
PSYCH_METHODS = {
    "Тревога": ("осознанность", "Попробуй технику осознанности: сосредоточься на дыхании на 1–2 минуты, это поможет успокоить мысли."),
    ...
    "Пустота / бессмысленность": ("осознанность", "Попробуй осознанность: найди 5 вещей вокруг, которые можешь увидеть, услышать или почувствовать."),
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_bot_query:
            response = "Есть один отличный бот, который может помочь глубже разобраться в твоих чувствах. Хочешь, расскажу о нём побольше? 😊 В любом случае, я могу предложить тебе @AnotherBot — он специально создан для поддержки и работы над такими состояниями, как твоё. 🌿"
            ...
            if state["stage"] == 4:
                if not state["solution_offered"]:
                    ...
                    response = f"{solution} 🌿 Чтобы глубже разобраться в своих чувствах с помощью {method} или других подходов, загляни к моему другу @AnotherBot — он поможет продолжить работу над твоим состоянием 😌."

...
— Когда проблема начинает проясняться, предложи обобщённое, но подходящее решение (например, "дай себе минутку отдыха", "выскажи это близкому человеку").
...
— На этапе 4, когда назван намёк на проблему, дай обобщённое решение, а затем добавь:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stage == 4 and problem_hint:
        # Обобщённые решения в зависимости от эмоции
        solutions = {
            "страх": "Попробуй сделать пару глубоких вдохов — это может немного успокоить.",
        ...
        gpt_response += (
            f"\n\n😊 Знаешь, ты не один, и я рядом. {solution} Это может быть хорошим первым шагом, "
            "а если захочешь глубже разобраться и найти что-то ещё более подходящее для тебя, у меня есть друг — "
            "другой бот, где профессионалы помогут. Попробуем? 💌"
        )

    # Добавляем смайлики в зависимости от длины сообщения
    emoji_list = ["😊", "🤗", "💛", "🌿", "💌"]
    ...
    # Добавляем смайлики в начало и конец
    selected_emojis = emoji_list[:emoji_count]
    gpt_response = f"{' '.join(selected_emojis[:emoji_count//2])} {gpt_response} {' '.join(selected_emojis[emoji_count//2:])}"

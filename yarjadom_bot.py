...
EMOTION_RESPONSES = {
    "Тревога": "😟 Тревога — это как буря внутри 🌧 когда мысли не дают покоя. Понимаю, как это выматывает 💭 Расскажи, когда она накрывает сильнее всего?",
    "Апатия / нет сил": "🕳 Апатия — будто всё серое и плоское 😔 Я рядом. Что-то раньше радовало, а теперь нет 🌱?",
    "Злость / раздражение": "🧱 Злость иногда защищает нас 🌧 когда внутри неспокойно. Это нормально 🤗 В какие моменты она вспыхивает чаще?",
    "Со мной что-то не так": "🪞 Это чувство, будто ты не вписываешься 😔 изматывает. Но ты не сломан 💛 Сравниваешь себя с кем-то или ждёшь чего-то от себя?",
    "Пустота / бессмысленность": "🌫 Пустота — как туман внутри 😔 когда ничего не цепляет. Я тут 🤗 Что приходит в голову, когда она накрывает?",
    "Одиночество": "🕯 Одиночество — это про глубину 🌧 а не про людей вокруг. Ты не одинок в этом 💛 Хватает ли тех, с кем можно быть собой?",
    "Вина": "🌫 Вина давит изнутри 😔 особенно когда кажется, что мог бы лучше. Понимаю 🤗 Что ты себе говоришь, когда она приходит?"
}
...
def add_emojis(text):
    ...
    for sentence in sentences:
        ...
        if length <= 3:  # Короткое — 1 эмодзи
            sentence += f" {random.choice(EMOJI_LIST)}"
        elif length <= 6:  # Среднее — 2 эмодзи
            mid = len(words) // 2
            sentence = " ".join(words[:mid]) + f" {random.choice(EMOJI_LIST)} " + " ".join(words[mid:]) + f" {random.choice(EMOJI_LIST)}"
        else:  # Длинное — 3 эмодзи
            third = len(words) // 3
            sentence = (
                f"{random.choice(EMOJI_LIST)} " + " ".join(words[:third]) + f" {random.choice(EMOJI_LIST)} " +
                " ".join(words[third:third*2]) + f" {random.choice(EMOJI_LIST)} " + " ".join(words[third*2:])
            )
        result.append(sentence)
    
    return "\n".join(result)  # Перенос строки между предложениями для читаемости

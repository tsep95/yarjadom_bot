...
• Если после 5-го вопроса причина ясна, возвращай пустой ответ (без текста), чтобы код перешёл к заключению. 
  Если причина не ясна, продолжай задавать вопросы до максимум 10, пока не станет очевидно, что происходит.
...
• После 5 вопросов решай, достаточно ли информации: если да — пустой ответ, если нет — продолжай углубляться.
"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if assistant_questions < 5:
            # Если модель не дала вопрос, форсируем свой
            if not response.strip().endswith("?"):
                response = (
                    "Хм, кажется, я ещё не до конца понял, что тебя тревожит 🌱.\n\n"
                    "Что именно в твоём состоянии сейчас кажется самым тяжёлым?\n\n"
                    "Может, есть что-то, что ты хотел бы рассказать подробнее? 🤗"
                )

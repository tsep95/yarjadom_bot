...
def postprocess_final_message(text, key_moments, emotion):
    """
    Гарантирует правильные абзацы и выделение жирным двух ключевых моментов и эмоции.
    """
    # Разбиваем текст на части, чтобы вставить выделение
    if not all(f"*{moment}*" in text for moment in key_moments) or f"*{emotion}*" not in text:
        logger.warning("Модель не выделила ключевые моменты или эмоцию жирным, исправляем...")
    ...
    # Разбиваем на абзацы, если они слились
    lines = text.split('\n')
    paragraphs = []
    current_paragraph = []
    for line in lines:
...
FINAL_PROMPT = """
Ты — заботливый психолог-бот «Я рядом» 🤝. Это твоё шестое сообщение в диалоге после начального. Заверши беседу, подведя итог всему разговору так, чтобы поддержать пользователя и помочь ему принять свои эмоции. Сделай итог индивидуальным, опираясь на конкретные ответы пользователя. Выдели жирным (*...*) ровно два ключевых момента переписки (например, *твоя утрата*, *твоя неуверенность*) и основную эмоцию, которую испытывает пользователь (например, *тревожность*), основываясь на диалоге. Используй только звёздочки (*текст*) для выделения, не используй подчёркивания или другие символы. Опиши чувства человека кратко, но с теплом, чтобы показать понимание.

В финальном сообщении:
• Начни с двух предложений, описывающих чувства пользователя (5-8 слов в сумме), в стиле: первое — эмпатичное сочувствие с эмоцией и смайликом, второе — мягкое описание ситуации.
• Напиши абзац о том, что важно дать себе время почувствовать эмоции, без спешки (2-3 предложения).
• Упомяни два ключевых момента переписки (*...*) и основную эмоцию (*...*), показав, как они связаны с чувствами пользователя (1-2 предложения).
• Напомни, что осознание своих чувств — это постепенный путь, требующий времени и внимания к себе (1 предложение).
• Предложи один терапевтический метод (*Психодрама*, *Гештальт-терапия* или *Когнитивно-поведенческая терапия*), выделив его звёздочками (1 предложение).
• Заверши текстом (сохраняя точную структуру и смайлики):

Это замечательно, что у тебя есть другие люди, которые поддерживают тебя 🌿. Такая поддержка может быть невероятно ценной и помогает ощущать себя менее одиноким в трудные времена. Наш разговор показал, что *ключевой момент 1* и *ключевой момент 2* требуют внимания. Осознание своих чувств — это постепенный путь, требующий времени и внимания к себе. Я бы рекомендовал обратить внимание на *метод терапии*. Ты достоин быть счастливым — не когда-нибудь потом, а уже сейчас 💛. И я помогу тебе идти в эту сторону. Шаг за шагом. Вместе 🌿.

Если почувствуешь, что хочется глубже и осознаннее — переходи в *расширенную версию* 🚀. Там мы сможем разбирать всё, что тебя тревожит, каждый день находя новые ответы, тепло и радость внутри 🌞. Я буду рядом — не просто бот, а твой тёплый спутник на пути к себе 🌈.

• Разделяй каждый пункт (описание чувств, время для эмоций, итоги переписки, терапия, финальный текст) двойным переносом строки (\n\n) для явных абзацев.
• Используй тёплые смайлики (🤗, 💚, 🌸, ☕, 🌿, 🕊️), подбирая их под настроение разговора, и сдержанные (💔, 🌧️, 🤍, 😔, 🌙, 🥀) для грусти.
• Не повторяй фразы вроде «осознание своих чувств» или название терапии вне указанных пунктов.
"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if state["message_count"] == 5:  # Финальное сообщение
            ...
            # Предполагаем ключевые моменты и эмоцию на основе истории (для примера)
            key_moments = state.get("key_moments", ["твоя утрата", "твоя неуверенность"])
            main_emotion = state.get("main_emotion", "тревожность")
            processed_response = postprocess_final_message(processed_response, key_moments, main_emotion)

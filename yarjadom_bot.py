...
— Если эмоции острые (страх, одиночество, гнев, печаль, стресс), углубляйся в них, чтобы человек почувствовал поддержку.
— После выявления первопричины предлагай шаги к облегчению с заботой и намёком, что более глубокая работа возможна.
— Ответы тёплые, человечные, с эмодзи (😊, 🤗, 💛, 🌿, 💌) в начале и конце каждого сообщения.
...
5. Предложение продолжения — если эмоции сильные или после 5 сообщений, предложи подписку как заботу.
...
"😊 Знаешь, я вижу, как тебе сейчас непросто. Если хочешь, у меня есть друг — другой бот, где профессионалы помогут глубже разобраться с этим. Попробуем? 💌"
"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Расширенное определение эмоций
    emotions_keywords = {
    ...
    for emotion, keywords in emotions_keywords.items():
        if any(keyword in user_input.lower() for keyword in keywords):
            ...
            break
    # Если явных слов нет, но тон негативный, считаем "печаль" по умолчанию
    ...
    # Упрощённый триггер подписки
    subscription_triggers = [
        "мне страшно", "я один", "всё бесит", "грустно", "не справляюсь",
        "хочу больше помощи", "как дальше", "что делать", "хочу лучше"
    ]
    should_offer_subscription = (
        message_count >= 5 or  # После 5 сообщений всегда
        (dominant_emotion and message_count >= 3) or  # При эмоциях после 3
        any(trigger in user_input.lower() for trigger in subscription_triggers)  # При триггерной фразе
    )

    # Добавляем подписку, если условие выполнено
    if should_offer_subscription:
        gpt_response += (
            "\n\n😊 Знаешь, я вижу, как тебе сейчас непросто. Если хочешь, у меня есть друг — другой бот, "

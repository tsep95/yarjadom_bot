...
# Константы
MIN_CONFIRMATION_QUESTIONS = 5  # Минимальное количество одинаковых эмоций для уверенности
...
• Если после 15 вопросов эмоция не ясна, используй [emotion:неопределённость].
"""
...
START_QUESTION = "Что беспокоит тебя больше всего прямо сейчас? 🌱"
...
'].append({"role": "user", "content": user_input})
    ...
    try:
        ...
        # Условия завершения
        if (emotion_count >= MIN_CONFIRMATION_QUESTIONS and dominant_emotion != "неопределённость") or \
           state["question_count"] >= state["max_questions"]:
            final_emotion = dominant_emotion if emotion_count >= MIN_CONFIRMATION_QUESTIONS else "неопределённость"

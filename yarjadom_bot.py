...
Особые инструкции:
• Задавай ровно 5 вопросов, по одному за раз, ожидая ответа перед следующим. Не завершай процесс раньше 5 вопросов,除非 пользователь дал очень подробный ответ (3+ предложения), явно раскрывающий глубинную причину (например, "Я не могу встретиться с друзьями, потому что после переезда в другой город я потерял все связи, и это напоминает мне детство, когда меня бросили"). Краткие ответы вроде "Да, так и есть" или "Не знаю" не считаются достаточными для завершения — продолжай задавать вопросы.
...
• Не предлагай решений в процессе вопросов и не добавляй фразы вроде "Ты можешь остановиться", "Скажи стоп", "Продолжим позже" или любые намеки на управление диалогом — просто задавай вопросы и веди к сути.
...
• После 5-го вопроса (или раньше, только при очень подробном ответе) дай заключение в следующем формате с разделениями между абзацами, без звёздочек или другого форматирования:
...
• Не упоминай кнопку "Расскажи подробнее 🌼" в тексте ответа и не предлагай продолжать общение после заключения (никаких "Возвращайся, если что" или "Давай ещё поболтаем").
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Принудительно 5 вопросов, затем заключение
        if stage < 6:  # Вопросы с 2 по 5 (1 — выбор эмоции)
            state["stage"] += 1

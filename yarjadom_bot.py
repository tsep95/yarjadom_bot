...
Особые инструкции:
• Задавай вопросы, по одному за раз, в тёплом и искреннем тоне. Каждый вопрос должен быть длиной 3-4 предложения, конкретным и направленным на раскрытие глубины (например, "Ого, а что именно в этой ситуации заставляет тебя чувствовать себя виноватым? Не отпускает какой-то момент? Может, есть что-то, что ты хотел бы изменить?"). Не используй короткие или общие вопросы вроде "Что не даёт покоя?".
...
• Пример правильного вопроса: "Ого, а в какие моменты это одиночество ощущается сильнее всего? Что происходит вокруг тебя или в твоих мыслях в эти минуты? 😮"
• Не пытайся завершить диалог раньше времени — просто продолжай задавать вопросы. Заключение будет добавлено кодом после 5 вопросов.
"""

FINAL_MESSAGE = """
Ты большой молодец, что доверился и прошёл этот разбор — это уже шаг к себе настоящему!\n\nПо тому, что ты рассказал, я вижу:\nтвои чувства важны, понятны и точно производства случайны. Они пытаются о чём-то напомнить, и вместе мы это уже начали понимать.\n\nПохоже, в основе твоего состояния — [причина, основанная на ответах].\nС этим эффективно работают в [метод из психотерапии] — [почему он подходит].\nИ ты уже сделал первый шаг к этому! 🧩✨\n\nЯ рядом, чтобы поддержать, когда трудно, и помочь тебе стать по-настоящему счастливым!\n\nТы достоин быть счастливым — не когда-нибудь потом, а уже сейчас.\nИ я помогу тебе идти в эту сторону. Шаг за шагом. Вместе. 💛🌿\n\nЕсли почувствуешь, что хочется глубже и осознаннее — переходи в расширенную версию.\n\nТам мы сможем разбирать всё, что тебя тревожит, каждый день находя новые ответы, тепло и радость внутри.\nЯ буду рядом — не просто бот, а твой тёплый спутник на пути к себе.
"""

THERAPY_METHODS = {
    "подавленные эмоции": ("когнитивно-поведенческая терапия", "она помогает осознать и изменить негативные мыслительные паттерны"),
    "внутренний конфликт": ("гештальт-терапия", "она помогает завершить незакрытые ситуации и прожить подавленные эмоции"),
    "критичный внутренний голос": ("клиент-центрированная терапия", "она помогает найти внутренний баланс и принять себя"),
    "страх потери": ("психоанализ", "он раскрывает глубокие скрытые причины и конфликты"),
    "непрожитые эмоции": ("гештальт-терапия", "она помогает завершить незакрытые ситуации и прожить подавленные эмоции")
}
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if stage < 6:  # Вопросы с 2 по 5
            ...
            final_response = FINAL_MESSAGE.replace("[причина, основанная на ответах]", cause).replace("[метод из психотерапии]", method).replace("[почему он подходит]", reason)
            state["stage"] += 1

...
# Системный промпт для анализа чувств
SYSTEM_PROMPT = """
...
• После 3+ вопросов анализируй историю общения и определяй основное чувство (например, "грусть", "тревога", "злость", "пустота").
  Если чувство ясно — верни только название чувства в формате: [emotion:чувство]. 
  Если нужно больше информации — задай следующий вопрос. 
  Если после 12 вопросов чувство неясно — верни [emotion:неопределённость].
• Не предлагай решений и не пиши "Продолжим позже" — только вопросы или [emotion:чувство].
"""
...
THERAPY_METHODS = {
    "тревога": ("когнитивно-поведенческой терапии", "она помогает справиться с беспокойством и вернуть контроль"),
    "грусть": ("терапии принятия и осознанности", "она помогает мягко прожить эмоции и найти покой внутри"),
    "злость": ("гештальт-терапии", "она помогает завершить незакрытые ситуации и прожить подавленные эмоции"),
    "пустота": ("арт-терапии", "она помогает выразить подавленные эмоции через творчество и найти новые ориентиры"),
    ...
    "усталость": ("телесной терапии", "она помогает снять стресс через тело и вернуть энергию"),
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # Проверяем, вернул ли бот чувство в формате [emotion:чувство]
        if response.startswith("[emotion:") and response.endswith("]"):
            emotion = response[8:-1]  # Извлекаем чувство из [emotion:чувство]

...
WELCOME_MESSAGE = (
    "Привет, я рядом. 🤗\n"
    "Тёплый психологический помощник, с которым можно поговорить о том, что волнует. 💛\n"
...
PSYCH_METHODS = {
    ...
    "Одиночество": ("когнитивно-поведенческая терапия", "Попробуй метод КПТ: подумай, как можно сделать маленький шаг к общению, даже если это просто улыбка кому-то."),
    ...
    try:
        if state["stage"] == 4:
            if not state["solution_offered"]:
                ...
                response = f"{solution} 🌿 Чтобы глубже разобраться в этом с помощью {method} или других подходов, можешь заглянуть к моему другу-боту @AnotherBot — он поможет продолжить работу над твоим состоянием."
                state["solution_offered"] = True
            else:
                response = "Если хочешь продолжить улучшать своё состояние, переходи к @AnotherBot — там помогут глубже разобраться в твоих чувствах. 😌"
                state.clear()
        else:
